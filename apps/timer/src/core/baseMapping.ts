...
export abstract class BaseMapping<T extends Model> {
  ...
  async sum(field: string, options = {}): Promise<number> {
    const res = await this.getModel().sum(field, options);
    return res;
  }

  async max(field: string, options = {}): Promise<unknown> {
    const res = await this.getModel().max(field, options);
    return res;
  }
}
